import { ref } from "vue";
import { tutorialMessages } from "./tutorialMessages.js";

const messages = {
  ja: {
    ui: {
      inventory: "インベントリ",
      menuInventory: "持ち物",
      menuVillagers: "村人",
      menuCraft: "クラフト",
      menuBuild: "建築",
      menuLog: "ログ",
      speed: "速度",
      minimap: "ミニマップ",
      logTitle: "ログ",
      cancel: "キャンセル",
      clear: "クリア",
      noLogs: "ログはまだありません。",
      player: "プレイヤー",
      playerName: "プレイヤー",
      villager: "村人",
      villagers: "村人一覧",
      container: "コンテナ",
      storageTargets: "在庫目標",
      addStorageTarget: "在庫目標を追加",
      noStorageTargets: "在庫目標はまだありません。",
      allItemsRegistered: "すべてのアイテムが登録済みです。",
      playerInspect: "プレイヤーのインベントリと現在のタスクを確認します。",
      playerTransferToContainer: "アイテムをクリックしてコンテナへ移動します。",
      containerTransferToPlayer: "アイテムをクリックしてプレイヤーへ移動します。",
      playerStatusInspect: "プレイヤーの状態とインベントリを確認します。",
      transferTo: "プレイヤー -> {target}",
      villagerInspect: "村人のインベントリ、担当設備、現在のタスクを確認します。",
      storage: "倉庫",
      groundItems: "地面のアイテム",
      storageInspect: "倉庫の中身と在庫目標を確認します。",
      groundItemsInspect: "倉庫が未建築のため、フィールドに見えているアイテムを表示しています。",
      moveNextToStorage: "アイテムを受け渡しするには倉庫に隣接してください。",
      moveNextToVillager: "アイテムを受け渡しするには村人に隣接してください。",
      village: "村人管理",
      villageInspect: "村人を追加し、現在の状態を確認します。",
      build: "建築",
      buildInspect: "フィールドに新しい建築予定地を配置します。",
      craftInspect: "手作業クラフトを行います。",
      facilityInspect: "村人の担当と自動作業の設定を行います。",
      close: "閉じる",
      eat: "食べる",
      drop: "ドロップ",
      currentTask: "現在のタスク",
      queue: "キュー",
      idle: "待機中",
      noActiveTask: "現在のタスクはありません。",
      handCraft: "手作業クラフト",
      busy: "作業中",
      ready: "準備完了",
      construction: "建築中",
      options: "設定",
      assignedStations: "担当設備",
      noAssignedStations: "担当設備はありません。",
      noStation: "担当なし",
      villagerInventory: "村人のインベントリ",
      stockEnabled: "有効",
      stockTarget: "目標数",
      stockLine: "在庫 {current} / 目標 {expected} / {status}",
      addVillager: "村人を追加",
      noVillagersAvailable: "追加できる村人はいません。",
      workbench: "作業台",
      lumberjackHut: "きこり小屋",
      add: "追加",
      remove: "外す",
      craftEntries: "クラフト登録",
      noCraftEntries: "クラフト登録はありません。",
      target: "目標",
      startCraftEntry: "この登録でクラフト開始",
      runningTasks: "進行中の作業",
      taskCount: "{count}件",
      noRunningTasks: "この設備で進行中の作業はありません。",
      fullness: "満腹度",
      starving: "飢餓",
      remainingSeconds: "残り {seconds}秒",
      leftSeconds: "残り {seconds}秒",
      addCraftEntry: "クラフト登録を追加",
      recipe: "レシピ",
      selectRecipe: "レシピを選択",
      confirm: "決定",
      assignVillager: "担当村人を追加",
      available: "利用可能",
      unavailable: "未開放",
      stockCurrent: "現在数",
      stockExpected: "目標数",
      stockTargetShort: "目標",
      currentOfTarget: "{current}/{target}",
      inventorySummary: "持ち物: {items}",
      pickupItemAction: "{item}を拾う",
    },
    taskPhase: {
      idle: "待機中",
      movingToTarget: "移動中",
      working: "作業中",
      movingToStorage: "搬入中",
    },
    status: {
      disabled: "無効",
      actionUnavailable: "条件不足",
      actionPending: "実行可能",
      actionDone: "達成済み",
      stationUnavailable: "設備未完成",
      noWorkers: "担当なし",
      running: "作業中",
      insufficientResources: "素材不足",
      workerBusy: "担当が作業中",
      invalidRecipe: "無効なレシピ",
      noTarget: "目標なし",
    },
    common: {
      handCraft: "手作業",
      facility: "設備",
      craft: "クラフト",
      build: "建築",
      gather: "採集",
      item: "アイテム",
      carryingNone: "何も持っていません",
      noAssignedStations: "担当設備なし",
    },
    task: {
      gathering: "{item}を採集中",
      building: "{building}を建築中",
      crafting: "{recipe}をクラフト中",
      transferring: "{item}を運搬中",
      eating: "{item}を食事中",
    },
    item: {
      wood: "木の棒",
      stone: "石",
      log: "丸太",
      wildBerry: "ワイルドベリー",
      workbench: "作業台",
      lumberjackHut: "きこり小屋",
      cookingStation: "調理場",
      storage: "倉庫",
      plank: "板材",
      stick: "木の棒",
      stoneAxe: "石斧",
      hammer: "ハンマー",
      berryJam: "ベリージャム",
    },
    recipe: {
      plank: "板材",
      stick: "木の棒",
      stoneAxe: "石斧",
      hammer: "ハンマー",
      berryJam: "ベリージャム",
    },
    station: {
      workbench: {
        name: "作業台",
        description: "村人を担当させて、板材や石斧などを自動でクラフトする設備です。",
      },
      lumberjackHut: {
        name: "きこり小屋",
        description: "村人を担当させて、石斧を使った木こり作業を自動化する設備です。",
      },
      cookingStation: {
        name: "調理場",
        description: "村人を担当させて、ベリージャムなどの料理を自動で作る設備です。",
      },
    },
    action: {
      pickupLog: "丸太を拾う",
      gatherWood: "木の棒を拾う",
      gatherStone: "石を拾う",
      gatherWildBerry: "ワイルドベリーを摘む",
      gatherLog: "木をこる",
    },
    building: {
      workbench: "作業台",
      storage: "倉庫",
      lumberjackHut: "きこり小屋",
      cookingStation: "調理場",
    },
    field: {
      tree: "木",
      branch: "木の棒",
      stone: "石",
      berryBush: "ベリーの茂み",
    },
    log: {
      gameStarted: "ゲームを開始しました。actor は対象に隣接してから作業します。",
      itemMissingAction: "{action}には{item}が必要です。{actor}の所持数: {count}",
      itemMissingActionShort: "{action}には{item}が必要です。",
      moveToPickup: "{actor}が移動を開始しました。",
      takeFromStorage: "{actor}が倉庫から{item}を取り出しました。",
      moveToAction: "{actor}が移動を開始しました。",
      villagerCannotStart: "{actor}は{item}を確保できず、{action}を開始できません。",
      gatherStarted: "{actor}が{action}を開始しました。{suffix}",
      autoSuffix: "在庫目標に向けた自動作業です。",
      playerCraftStarted: "プレイヤーが{recipe}のクラフトを開始しました。",
      actorAte: "{actor}が{item}を食べました。",
      tooHungryToWork: "{actor}は空腹で、食事以外の行動ができません。",
      noVillagerForStation: "{station}で作業できる村人がいません。",
      villagerCraftStarted: "{actor}が{recipe}のクラフトを開始しました。{suffix}",
      playerBuildStarted: "プレイヤーが{building}の建築を開始しました。",
      villagerBuildStarted: "{actor}が{building}の建築を開始しました。",
      craftFailedMissingResources: "{recipe}の完了時に素材が不足していました。",
      craftCompletePlayer: "{recipe}が完成し、プレイヤーのインベントリに入りました。",
      craftCompleteStorage: "{recipe}が完成し、倉庫へ搬入されました。",
      gatherDelivered: "{item}を{amount}個、倉庫へ搬入しました。",
      buildingComplete: "{building}の建築が完了しました。",
      buildingCancelled: "{building}の建築を撤去し、資材を地面に戻しました。",
      choppedTree: "{actor}が木をこり、周囲に丸太が落ちました。",
      pickedUpItem: "{actor}が{item}を拾いました。",
      moveToStorage: "{actor}が{item}を持って倉庫へ向かっています。",
      moveCraftToStorage: "{actor}が{item}を持って倉庫へ向かっています。",
      placedBuilding: "{building}を配置しました。必要資材を消費して建築待ちです。",
      assignedStation: "{actor}を{station}の担当にしました。",
      unassignedStation: "{actor}を{station}の担当から外しました。",
      villagerJoined: "{actor}が村に加わりました。",
      craftEntryAdded: "{station}に{recipe}を登録しました。",
      craftEntryRemoved: "{station}から{recipe}の登録を外しました。",
      moveItemToContainer: "{actor}が{item}を{container}へ移しました。",
      moveItemFromContainer: "{actor}が{container}から{item}を取り出しました。",
      taskCancelled: "{task} をキャンセルしました。",
    },
  },
  en: {
    ui: {
      inventory: "Inventory",
      menuInventory: "Inventory",
      menuVillagers: "Villagers",
      menuCraft: "Craft",
      menuBuild: "Build",
      menuLog: "Log",
      speed: "Speed",
      minimap: "Minimap",
      logTitle: "Log",
      cancel: "Cancel",
      clear: "Clear",
      noLogs: "No logs yet.",
      player: "Player",
      playerName: "Player",
      villager: "Villager",
      villagers: "Villagers",
      container: "Container",
      storageTargets: "Storage Targets",
      addStorageTarget: "Add Storage Target",
      noStorageTargets: "No storage targets yet.",
      allItemsRegistered: "All items are already registered.",
      playerInspect: "Inspect the player inventory and current task.",
      playerTransferToContainer: "Click an item to move it into the container.",
      containerTransferToPlayer: "Click an item to move it into the player inventory.",
      playerStatusInspect: "Player status and inventory.",
      transferTo: "Player -> {target}",
      villagerInspect: "Inspect this villager inventory, assignments, and current task.",
      storage: "Storage",
      groundItems: "Ground Items",
      storageInspect: "Review stored items and stock targets.",
      groundItemsInspect: "Storage is not built yet, so visible field items are shown here.",
      moveNextToStorage: "Move next to the storage to transfer items.",
      moveNextToVillager: "Move next to the villager to transfer items.",
      village: "Villager Management",
      villageInspect: "Add villagers and inspect their current state.",
      build: "Construction",
      buildInspect: "Place a new construction site on the field.",
      craftInspect: "Craft items by hand.",
      facilityInspect: "Assign villagers and configure automatic production.",
      close: "Close",
      eat: "Eat",
      drop: "Drop",
      currentTask: "Current Task",
      queue: "Queue",
      idle: "Idle",
      noActiveTask: "No active task.",
      handCraft: "Hand Craft",
      busy: "Busy",
      ready: "Ready",
      construction: "Construction",
      options: "Options",
      assignedStations: "Assigned Stations",
      noAssignedStations: "No assigned stations.",
      noStation: "No station",
      villagerInventory: "Villager Inventory",
      stockEnabled: "Enabled",
      stockTarget: "Target",
      stockLine: "Stock {current} / Expected {expected} / {status}",
      addVillager: "Add Villager",
      noVillagersAvailable: "No villagers available.",
      workbench: "Workbench",
      lumberjackHut: "Lumberjack Hut",
      add: "Add",
      remove: "Remove",
      craftEntries: "Craft Queue",
      noCraftEntries: "No craft entries.",
      target: "Target",
      startCraftEntry: "Start Crafting",
      runningTasks: "Running Tasks",
      taskCount: "{count} tasks",
      noRunningTasks: "No running tasks for this facility.",
      fullness: "Fullness",
      starving: "Starving",
      remainingSeconds: "{seconds}s left",
      leftSeconds: "{seconds}s left",
      addCraftEntry: "Add Craft Entry",
      recipe: "Recipe",
      selectRecipe: "Select a recipe",
      confirm: "Confirm",
      assignVillager: "Assign Villager",
      available: "Available",
      unavailable: "Locked",
      stockCurrent: "Stock",
      stockExpected: "Expected",
      stockTargetShort: "Target",
      currentOfTarget: "{current}/{target}",
      inventorySummary: "Inventory: {items}",
      pickupItemAction: "Pick up {item}",
    },
    taskPhase: {
      idle: "Idle",
      movingToTarget: "Moving",
      working: "Working",
      movingToStorage: "Delivering",
    },
    status: {
      disabled: "Disabled",
      actionUnavailable: "Requirements missing",
      actionPending: "Ready",
      actionDone: "Target reached",
      stationUnavailable: "Facility incomplete",
      noWorkers: "No workers",
      running: "Working",
      insufficientResources: "Missing resources",
      workerBusy: "Workers busy",
      invalidRecipe: "Invalid recipe",
      noTarget: "No target",
    },
    common: {
      handCraft: "Hand Craft",
      facility: "Facility",
      craft: "Craft",
      build: "Build",
      gather: "Gather",
      item: "item",
      carryingNone: "Carrying nothing",
      noAssignedStations: "No assigned stations",
    },
    task: {
      gathering: "Gathering {item}",
      building: "Building {building}",
      crafting: "Crafting {recipe}",
      transferring: "Transporting {item}",
      eating: "Eating {item}",
    },
    item: {
      wood: "Branch",
      stone: "Stone",
      log: "Log",
      wildBerry: "Wild Berry",
      workbench: "Workbench",
      lumberjackHut: "Lumberjack Hut",
      cookingStation: "Cooking Station",
      storage: "Storage",
      plank: "Plank",
      stick: "Stick",
      stoneAxe: "Stone Axe",
      hammer: "Hammer",
      berryJam: "Berry Jam",
    },
    recipe: {
      plank: "Plank",
      stick: "Stick",
      stoneAxe: "Stone Axe",
      hammer: "Hammer",
      berryJam: "Berry Jam",
    },
    station: {
      workbench: {
        name: "Workbench",
        description: "Assign villagers to automatically craft planks, stone axes, and more.",
      },
      lumberjackHut: {
        name: "Lumberjack Hut",
        description: "Assign villagers to automatically chop trees with stone axes.",
      },
      cookingStation: {
        name: "Cooking Station",
        description: "Assign villagers to automatically cook food such as berry jam.",
      },
    },
    action: {
      pickupLog: "Pick up log",
      gatherWood: "Pick up branch",
      gatherStone: "Pick up stone",
      gatherWildBerry: "Pick wild berry",
      gatherLog: "Chop tree",
    },
    building: {
      workbench: "Workbench",
      storage: "Storage",
      lumberjackHut: "Lumberjack Hut",
      cookingStation: "Cooking Station",
    },
    field: {
      tree: "Tree",
      branch: "Branch",
      stone: "Stone",
      berryBush: "Berry Bush",
    },
    log: {
      gameStarted: "The game has started. Actors move next to targets before they work.",
      itemMissingAction: "{item} is required for {action}. {actor} holds: {count}",
      itemMissingActionShort: "{item} is required for {action}.",
      moveToPickup: "{actor} started moving.",
      takeFromStorage: "{actor} took {item} from storage.",
      moveToAction: "{actor} started moving.",
      villagerCannotStart: "{actor} could not secure {item} and cannot start {action}.",
      gatherStarted: "{actor} started {action}. {suffix}",
      autoSuffix: "This is automatic work toward a stock target.",
      playerCraftStarted: "The player started crafting {recipe}.",
      actorAte: "{actor} ate {item}.",
      tooHungryToWork: "{actor} is too hungry to do anything except move and eat.",
      noVillagerForStation: "No villager is available to work at {station}.",
      villagerCraftStarted: "{actor} started crafting {recipe}. {suffix}",
      playerBuildStarted: "The player started building {building}.",
      villagerBuildStarted: "{actor} started building {building}.",
      craftFailedMissingResources: "Materials were missing when completing {recipe}.",
      craftCompletePlayer: "{recipe} is complete and was added to the player inventory.",
      craftCompleteStorage: "{recipe} is complete and was delivered to storage.",
      gatherDelivered: "{amount} {item} delivered to storage.",
      buildingComplete: "{building} construction is complete.",
      buildingCancelled: "{building} construction was removed and the materials were dropped on the ground.",
      choppedTree: "{actor} chopped a tree and logs dropped nearby.",
      pickedUpItem: "{actor} picked up {item}.",
      moveToStorage: "{actor} is carrying {item} to storage.",
      moveCraftToStorage: "{actor} is carrying {item} to storage.",
      placedBuilding: "{building} was placed. Resources were consumed and it is waiting for construction.",
      assignedStation: "{actor} was assigned to {station}.",
      unassignedStation: "{actor} was removed from {station}.",
      villagerJoined: "{actor} joined the village.",
      craftEntryAdded: "{recipe} was added to {station}.",
      craftEntryRemoved: "{recipe} was removed from {station}.",
      moveItemToContainer: "{actor} moved {item} to {container}.",
      moveItemFromContainer: "{actor} took {item} from {container}.",
      taskCancelled: "{task} was cancelled.",
    },
  },
};

mergeMessages(messages, tutorialMessages);

export const currentLocale = ref("ja");

function lookup(locale, key) {
  return key.split(".").reduce((value, segment) => value?.[segment], messages[locale]);
}

export function t(key, params = {}) {
  const template = lookup(currentLocale.value, key) ?? lookup("ja", key) ?? key;
  return template.replace(/\{(\w+)\}/g, (_, name) => String(params[name] ?? `{${name}}`));
}

export function setLocale(locale) {
  if (messages[locale]) {
    currentLocale.value = locale;
  }
}

export function useI18n() {
  return {
    locale: currentLocale,
    setLocale,
    t,
  };
}

function mergeMessages(target, source) {
  Object.entries(source).forEach(([locale, values]) => {
    if (!target[locale]) {
      target[locale] = {};
    }
    mergeRecord(target[locale], values);
  });
}

function mergeRecord(target, source) {
  Object.entries(source).forEach(([key, value]) => {
    if (value && typeof value === "object" && !Array.isArray(value)) {
      if (!target[key] || typeof target[key] !== "object" || Array.isArray(target[key])) {
        target[key] = {};
      }
      mergeRecord(target[key], value);
      return;
    }
    target[key] = value;
  });
}
