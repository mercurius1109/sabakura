<template>
  <section>
    <div
      class="mb-3 text-sm font-bold tracking-[0.08em]"
      :class="tone === 'dark' ? 'text-white/[0.92]' : 'text-white/[0.92]'"
    >
      {{ caption }}
    </div>

    <div
      v-if="entries.length === 0"
      class="text-sm"
      :class="tone === 'dark' ? 'text-white/[0.66]' : 'text-white/[0.66]'"
    >
      {{ emptyText }}
    </div>

    <div v-else class="grid grid-cols-[repeat(auto-fill,minmax(72px,72px))] gap-3">
      <button
        v-for="entry in entries"
        :key="entry.id"
        type="button"
        :title="`${entry.name} x${entry.amount}`"
        :disabled="disabled"
        :class="disabled
          ? 'relative flex h-[72px] w-[72px] cursor-not-allowed items-center justify-center rounded-lg border border-white/[0.12] bg-black/[0.34] backdrop-blur-sm'
          : 'relative flex h-[72px] w-[72px] items-center justify-center rounded-lg border border-white/[0.12] bg-black/[0.34] backdrop-blur-sm transition hover:-translate-y-0.5'"
        @click="handleSelect(entry.id)"
      >
        <span
          v-if="disabled"
          class="pointer-events-none absolute right-1 top-1 z-10 flex h-4 w-4 items-center justify-center rounded-full bg-black/[0.68] text-[10px] leading-none text-white/[0.92]"
          aria-hidden="true"
        >
          ⃠
        </span>
        <span class="pointer-events-none absolute left-1.5 top-1.5 z-10 max-w-[46px] truncate text-[10px] font-bold leading-4 text-white/[0.92]">
          {{ entry.name }}
        </span>
        <div class="flex h-[72px] w-[72px] items-center justify-center p-2" aria-hidden="true">
          <GameIcon :icon="entry.icon" :alt="entry.name" />
        </div>
        <span
          v-if="entry.stackable !== false && entry.amount > 1"
          class="pointer-events-none absolute bottom-0.5 right-1 z-10 text-[10px] font-normal leading-4 text-white/[0.92]"
        >
          ×{{ entry.amount }}
        </span>
        <span class="sr-only">{{ entry.name }}</span>
      </button>
    </div>

    <div
      v-if="disabled && entries.length > 0 && disabledText"
      class="mt-3 text-sm"
      :class="tone === 'dark' ? 'text-white/[0.66]' : 'text-white/[0.66]'"
    >
      {{ disabledText }}
    </div>
  </section>
</template>

<script setup>
import GameIcon from "./GameIcon.vue";

const props = defineProps({
  caption: { type: String, required: true },
  emptyText: { type: String, required: true },
  entries: { type: Array, required: true },
  disabled: { type: Boolean, default: false },
  disabledText: { type: String, default: "" },
  tone: { type: String, default: "light" },
});

const emit = defineEmits(["select"]);

function handleSelect(entryId) {
  if (!props.disabled) {
    emit("select", entryId);
  }
}
</script>
